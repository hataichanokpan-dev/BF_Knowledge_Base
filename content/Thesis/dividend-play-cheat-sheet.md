---
title: "Dividend Play - Cheat Sheet"
type: cheatsheet
created: 2026-04-04
tags:
  - thesis
  - quick-reference
  - dividend
---

# Dividend Play - Cheat Sheet

> **Quick Reference สำหรับการตัดสินใจหุ้นปันผลไทย**

---

## ✅ PASS Criteria (ต้องผ่านทั้งหมด)

| Metric | Threshold | คำนวณ |
|--------|-----------|-------|
| Dividend Yield | > 5% | Yield = DPS / Price |
| Payout Ratio | < 80% | Payout = DPS / EPS |
| FCF Cover | > 1.2x | FCF Cover = FCF / DPS |
| ROE | > 10% | ROE = NI / Equity |
| D/E Ratio | < 1.0 | D/E = Debt / Equity |
| VTS Score | < 8 | ดูตารางด้านล่าง |

---

## 🚫 REJECT Criteria (พบข้อใดข้อ = หลีกเลี่ยง)

| Red Flag | Score | Trigger |
|---------|-------|--------|
| Payout > 90% ต่อเนื่อง 2 ปี | 8 | Unsustainable dividend |
| FCF Cover < 1.0x | 10 | Borrowing to pay dividend |
| กำไรถดถอย 3 ปีติดต่อ | 9 | Earnings erosion |
| หนี้เพิ่ม + กำไรลด | 7 | Worsening financial position |
| ประวัติลดปันผล | 8 | Management credibility issue |
| Structural Decline | 9 | Industry disruption |

**VTS ≥ 15 = 🚫 REJECT**

---

## ⚠️ CAUTION Criteria (ตรวจสอบเพิ่ม)

| Yellow Flag | Score | Action |
|------------|-------|--------|
| Payout 70-90% | 4 | Check CAPEX needs |
| ROE ลดลงต่อเนื่อง | 4 | Efficiency declining |
| Working Capital ติดลบ | 3 | Liquidity concern |

**VTS 8-14 = ⚠️ DEEP DIVE REQUIRED**

---

## 📐 Position Sizing

| Tier | Base | Criteria | Conviction Multiplier |
|------|------|----------|----------------------|
| **A** | 8% | Yield > 6%, Payout < 60%, FCF > 1.5x | 1.0x (Standard) |
| **B** | 6% | Yield 5-6%, Payout 60-70%, FCF 1.2-1.5x | 0.75x (Yellow Flags) |
| **C** | 4% | Yield 4-5%, Payout 70-80%, FCF 1.0-1.2x | 0.5x (Red Flags < 8) |

**Final Position = Base × Conviction Multiplier × MOS Adjustment**

---

## 🛡️ Stop Loss Rules

| Trigger | Stop Loss | Action |
|---------|-----------|--------|
| **Thesis Break** | ทันที | Sell all |
| **Dividend Cut** | ทันที | Sell all |
| **Technical Stop** | -15% | Evaluate for thesis break |

---

## 💰 Take Profit Rules

| Trigger | Take Profit | Action |
|---------|-------------|--------|
| **Overvalued** (P/E > 20x) | 50% position | Sell half |
| **Yield Compression** (< 3%) | ทบทวน | Consider selling |
| **Target Price** (IV + 20%) | ทั้งหมด | Sell all |

---

## 📅 Sector Limits

| Rule | Limit | Why |
|------|------|-----|
| Max per Sector | 25% | Reduce sector-specific risk |
| Max per Sub-Sector | 15% | REITs within Property |
| Min Sectors | 3 | Diversification |

---

## 🌡️ Rate Sensitivity Quick Check

| Sector | Rate Rising | Rate Falling |
|--------|-------------|--------------|
| **REITs** | ❌ Avoid | ✅ Buy |
| **Banks** | ⚠️ Caution | ✅ Good |
| **Power** | ⚠️ Caution | ✅ Buy |
| **Telco** | ⚠️ Caution | ✅ Good |

---

## 💰 Net Yield Calculator

```
Net Yield = Gross Yield × (1 - WHT)

WHT Rates:
- บุคคลทั่วไป (ใหม่): 0%
- นิติบุคคลธรรม (ใหม่): 0%
- บุคคลทั่วไป (เดิม): 10%
- นิติบุคคลธรรม (เดิม): 10%
- ต่างชาติ: 10-15%
```

---

## 📅 Seasonal Timing

| Month | Action |
|-------|--------|
| **Nov-Feb** | Build watchlist, start accumulating |
| **Mar** | Final entries, watch XD announcements |
| **Apr** | HOLD through XD, collect dividends |
| **May** | Receive dividends, evaluate rebalancing |
| **Jun-Oct** | Research phase, build next year's list |

---

## 🔗 Quick Links

- [[Dividend-Play-Thesis]] - Full Thesis v2.0
- [[Dividend-Play-Flow]] - Visual Workflows
- [[Litzenberger-Ramaswamy-1979-1982]] - Dividend Premium Evidence
